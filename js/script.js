...
/*
            .then(data => {
              ...
              // takes javascript API data and replaces javascript HTML element contents

              // OPTION !:
              todayUVI = document.getElementById('city-uvi-today');
              todayUVI.innerHTML = `
              <p id="city-uvi-today">UV Index: <span id="uvi-badge" class="badge text-dark">${uvIndex}</span></p>
              `

              // OPTION 2:
              document.getElementById('city-uvi-today').innerHTML = `
              <p id="city-uvi-today">UV Index: <span id="uvi-badge" class="badge text-dark">${uvIndex}</span></p>
              `
              cityUVIndex.innerHTML = `
              <p id="city-uvi-today">UV Index: <span id="uvi-badge" class="badge text-dark">${uvIndex}</span></p>
              ...
              cityUVIndex.innerHTML = `hi`

              console.log("UVIndex var API: ", data);
              console.log("cityUVIndex var is: ", uvIndex);
*/
...
// *** Event listener
// stops page reload on submit, passes input value to setlocalStorage(), then empties the input search field
...
// ? SET:
// ? user input is labeled as 'citySearch'.
// ? 'citySearch' item is push into 'searchHistoryArray'
// ? 'searchHistoryArray' is then saved in localStorage under the name 'citySearchHistory'

// ? now local storage should have an array called 'citySearchHistory', and its indexed items do not need a name?
// ? then we can JSON.parse to get all the cities?

// ? GET:
// ? we want 'citySearchHistory' from localStorage
// ? we make the saved array a variable called 'cityHistory' that we can play with
// ???? run an IF statement through the array to get the list of saved cities
// ???? then we can end function by sending index values to renderHistory() for user display

// *** gets localStorage
// grab cityHistory value; if a value exists, parse back into array, send to renderHistory()
function getLocalStorage() {
  const cityHistory = localStorage.getItem('citySearchHistory');
  if (cityHistory) {
...
//   // clears existing <ul> so we don't duplicate stuff, then run forEach()
...
//     liNewEl.innerHTML = `
//     <li class="list-group-item search-city">${citySearchHistory}</li> 
//     `;
//     // ! ?? but, cSH is an array, right?

//     //! also need to add <a> or something to innerHTML so a click searches again for city
